...
public class PostService {
	...
	public void update(PostUpdateDto dto) {
		...
		// @Transactional 에너테이션을 사용할 경우,
		// DB에서 검색한 entitiy 객체가 변경되면 update 쿼리가 자동으로 실행.
...
//		String keyword = dto.getKeyword();
		...
		/*
		 * if(dto.getCategory().equals("t")) { result =
		 * postRepo.findByTitleContainingIgnoreCase(keyword, pageable); } else if
		 * (dto.getCategory().equals("c")) { result =
		 * postRepo.findByContentContainingIgnoreCase(keyword, pageable); } else if
		 * (dto.getCategory().equals("tc")) { result =
		 * postRepo.findByTitleOrContent(keyword, pageable); } else if
		 * (dto.getCategory().equals("a")) { result =
		 * postRepo.findByAuthorContainingIgnoreCase(keyword, pageable); } else { return
		 * null; }
		 */
