 ...
 * 2. 프로젝트 이름 오른쪽 클릭 -> Gradle -> Refresh Gradle Projecy
 * 3. Window -> Show View -> other... -> Gradle Tasks, Gradle Excutions Open
 * 4. Gradle Tasks -> 프로젝트 -> build -> clean 오른쪽 클릭 -> Run Gradle Tasks
 * 5. Gradle Tasks -> 프로젝트 -> build -> build 오른쪽 클릭 -> Run Gradle Tasks
 * 6. 프로젝트 이름 오른쪽 클릭 -> Gradle -> Refresh Gradle Projecy
 * 		build/generated/querydsl 폴더에 com.itwill.springboot5.domain 패키지가 생성
 * 		패키지에는 Q클래스들이 자동 생성.
 * 7. 인터페이스(예: PostQuerydsl) 작성
 * 8. 인터페이스를 구현하는 클래스를(PostQuerydslImpl) 작성.
 * 	  (1) QuerydslRepositorySupport 상속
 * 		  - 상위클래스가 기본 생성자를 갖고 잊지 않기 때문에, 아규먼트를 갖는 super(arg)를 명시적으로 호출.
...
public interface PostQuerydsl {
	...
	// title에 포함된 문자열 대소문자 구분없이 검색하기
	List<Post> searchByTitle(String keyword);
	
	// content에 포함된 문자열 대소문자 구분없이 검색하기
