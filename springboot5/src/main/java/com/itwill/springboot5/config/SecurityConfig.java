...
public class SecurityConfig {
	...
	// 스프링 부트 애플리케이션에서 스플링 시큐리티를 이용한 로그인/로그아웃을 하려면
	// UserDetailsService 인터페이스를 구현하는 서비스 클래스와 
	// UserDetails 인터페이스를 구현하는 엔터티 클래스가 있어야 함.
	// 사용자 엔터티의 사용자 서비스를 구현하기 전에 테스트 용도로 사용할 코드.
	/*
	@Bean
	...
	*/
	...
	SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		...
		http.authorizeHttpRequests((auth) -> 
			...
			auth.requestMatchers("/post/create", "/post/details", "/post/modify", "/post/delete", "/post/update"
					, "/api/comment/**")
				//.authenticated()
				.hasRole("USER")
				.anyRequest()
