...
public class HomeController {
	...
	public void bookdetails(Model model, @RequestParam(name="id") int id) {
		// 요청 파라미터 id 값을 찾고, 해당 id를 갖는 Book 객체를 만듦.
		// 모델에 Book 객체를 속성(attr)으로 저장. 뷰로 전달
		
		log.info("bookDetails(id={})",id);
		
		Book book = Book.builder().id(id).title("종의 기원")
	...
	public String bookdetails2(@PathVariable(name="id") int id, Model model) {
		...
		log.info("bookDetails2(pathVariable={})",id);
		
		Book book = Book.builder().id(id).title("개미")
				.author(Author.builder().firstName("베르나르").lastName("베르베르").build())
				.build();
