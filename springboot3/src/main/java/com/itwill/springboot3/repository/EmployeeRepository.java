...
public interface EmployeeRepository extends JpaRepository<Employee, Integer> {
	...
	// 급여가 어떤 값 미만인 직원들의 정보(where salart < ?)
	...
	// 엔티티 객체 이름과 엔티티 필드 이름으로 쿼리를 작성하는 문법.
	// alias(별명)을 반드시 사용해야 함.
	// 엔티티 이름과 필드 이름은 대소문자를 구분.
	...
	@Query("select e from Employee e join fetch e.department d where d.location.country.countryName = :country")
	List<Employee> findByTest(@Param("country") String countryName);
